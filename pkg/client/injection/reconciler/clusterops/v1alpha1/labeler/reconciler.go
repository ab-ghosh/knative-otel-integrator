...
// controller reconciling v1alpha1.Labeler.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Labeler. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Labeler) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Labeler.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Labeler. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Labeler) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Labeler if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Labeler.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Labeler) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Labeler) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Labeler resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister clusteropsv1alpha1.LabelerLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Labelers(s.namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Labeler, desiredFinalizers sets.Set[string]) (*v1alpha1.Labeler, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.ClusteropsV1alpha1().Labelers(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Labeler) (*v1alpha1.Labeler, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Labeler, reconcileEvent reconciler.Event) (*v1alpha1.Labeler, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
